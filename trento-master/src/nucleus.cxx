...
NucleusPtr Nucleus::create(const std::string& species, double nucleon_dmin) {
  ...
  else if (species == "Ru96") ///Farid Added
    return NucleusPtr{new DeformedWoodsSaxonNucleusII{
       96, 5.09, 0.46, 0.162, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "RuII") ///Farid Added
    return NucleusPtr{new DeformedWoodsSaxonNucleusII{
       96, 5.09, 0.46, 0.06, 0.0, 0.0, nucleon_dmin
    }};
  else if (species == "RuIII") ///Farid Added
    return NucleusPtr{new DeformedWoodsSaxonNucleusII{
       96, 5.09, 0.46, 0.06, 0.20, 0.0, nucleon_dmin
    }};
  else if (species == "RuIV") ///Farid Added
    return NucleusPtr{new DeformedWoodsSaxonNucleusII{
       96, 5.09, 0.52, 0.06, 0.20, 0.0, nucleon_dmin
    }};
    ///==================
  else if (species == "Zr96") ///Farid Added
    return NucleusPtr{new DeformedWoodsSaxonNucleusII{
       96, 5.02, 0.52, 0.06, 0.20, 0.0, nucleon_dmin
    }};
    ///==================
    /// ======= Giulinao setup ==============
     else if (species == "Ru_cj")
      return NucleusPtr{new DeformedWoodsSaxonNucleusII{
        96, 5.04, 0.47, 0.16, 0.0, 0.0, nucleon_dmin
      }};
    else if (species == "Zr_cj")
      return NucleusPtr{new DeformedWoodsSaxonNucleusII{
        96, 4.97, 0.54, 0.06, 0.17, 0.0, nucleon_dmin
      }};
///=============================
///======== EMMI Task===========
 else if (species == "Case2")
  return NucleusPtr{new DeformedWoodsSaxonNucleusII{
    96, 5.09, 0.46, 0.16, 0.0, 0.0, nucleon_dmin
  }};
 else if (species == "Case3")
  return NucleusPtr{new DeformedWoodsSaxonNucleusII{
    96, 5.09, 0.46, 0.16, 0.2, 0.0, nucleon_dmin
  }};
   else if (species == "Case4")
  return NucleusPtr{new DeformedWoodsSaxonNucleusII{
    96, 5.09, 0.46, 0.06, 0.2, 0.0, nucleon_dmin
  }};
    else if (species == "Case5")
  return NucleusPtr{new DeformedWoodsSaxonNucleusII{
    96, 5.09, 0.52, 0.06, 0.2, 0.0, nucleon_dmin
  }}; 

  else if (species == "Case6")
  return NucleusPtr{new DeformedWoodsSaxonNucleusII{
    96, 5.02, 0.52, 0.06, 0.2, 0.0, nucleon_dmin
  }};  
///=============================      
...
///<<<<<<<<<Farid Added >>>>>>>>>>>>>>.
DeformedWoodsSaxonNucleusII::DeformedWoodsSaxonNucleusII(
    std::size_t A, double R, double a, double beta2, double beta3, double beta4, double dmin)
    : MinDistNucleus(A, dmin),
      R_(R),
      a_(a),
      beta2_(beta2),
...
double DeformedWoodsSaxonNucleusII::radius() const {
  return rmax_ - 7.*a_;
}
...
double DeformedWoodsSaxonNucleusII::deformed_woods_saxon_distII(
    double r, double cos_theta) const {
  auto cos_theta_sq = cos_theta*cos_theta;
  ...
  using math::double_constants::one_div_root_pi;
  auto Y20 = std::sqrt(5)/4. * one_div_root_pi * (3.*cos_theta_sq - 1.);
  auto Y30 = std::sqrt(7)/4. * one_div_root_pi * (5.*cos_theta_sq*cos_theta - 3.*cos_theta);
  auto Y40 = 3./16. * one_div_root_pi *
             (35.*cos_theta_sq*cos_theta_sq - 30.*cos_theta_sq + 3.);
...
void DeformedWoodsSaxonNucleusII::sample_nucleons_impl() {
   // The deformed W-S distribution is defined so the symmetry axis is aligned
  // with the Z axis, so e.g. the long axis of uranium coincides with Z.
  //
  // After sampling positions, they must be randomly rotated.  In general this
  ...
  for (auto&& sample : samples) {
    ...
    do {
      ...
      random::canonical<double>() >
      deformed_woods_saxon_distII(sample.r, sample.cos_theta)
    );
  }
