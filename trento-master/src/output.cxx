...
void write_header(std::ostream& os_f, int width) {
    using std::fixed;
  using std::setw;
  
  os_f << "#"<< setw(width) <<   setw(5)   << "b" <<  fixed    << setw(15) << "npart" <<  setw(15)  
   << "multi" << setw(18) <<  "ex2" <<  setw(15) << "ex3" <<  setw(15) << "ex4" << setw(15) <<  "ex5" 
   <<  setw(15) << "ey2" <<  setw(15) <<  "ey3" <<  setw(15) <<  "ey4" <<  setw(15) <<  "ey5" <<  setw(15)
   <<  "r2" <<  setw(17) <<  "r3" <<  setw(17) <<  "r4" <<  setw(17) <<  "r5"<< '\n';
 } 

void write_header_long(std::ostream& os_f, int width) {
    using std::fixed;
  using std::setw;
  
  os_f << "#"<< setw(width) <<   setw(5)   << "b" <<  fixed    << setw(15) << "npart" <<  setw(15)  
   << "multi" << setw(18) 
   <<  "ex2" <<  setw(15) << "ex3" <<  setw(15) << "ex4" << setw(15) <<  "ex5"  << setw(15) <<  "ex6"  << setw(15) <<  "ex7"  << setw(15) <<  "ex8" <<  setw(15)
   <<  "ey2" <<  setw(15) << "ey3" <<  setw(15) << "ey4" << setw(15) <<  "ey5"  << setw(15) <<  "ey6"  << setw(15) <<  "ey7"  << setw(15) <<  "ey8" <<  setw(15)
   <<  "r2" <<  setw(17) <<  "r3" <<  setw(17) <<  "r4" <<  setw(17) <<  "r5" <<  setw(17) <<  "r6"  <<  setw(17) <<  "r7"  <<  setw(17) <<  "r8"
   << '\n';
 }  
//!===========  =====================

//  <<<<<<<<< ! Farid modeifed the following: >>>>>>>>>>
...
//  os << setw(5) << "|";
 //!      <<<<<<<<<<<<<   Farid added  >>>>>>>>>>>>>>>>
 // os << setw(5) << "  ex,2,3,4,5:";

    for (const auto& ex : event.eccx())
      os << setw(15)   << ex.second;
    
   // os << setw(5) << "  ey,2,3,4,5:";

    for (const auto& ey : event.eccy())
      os << setw(15)   << ey.second ;

   // os << setw(5) << "  rm,2,3,4,5:";

    for (const auto& farid_rm : event.rm())
      os << setw(20)   << farid_rm.second;  
  //! =================================================
...
void write_text_file(const fs::path& output_dir, int width, int num,
 ...
 //!      <<<<<<<<<<<<<   Farid added  >>>>>>>>>>>>>>>>
    for (const auto& ex : event.eccx()){
      ofs << "# ex" << ex.first << "    = " << ex.second << '\n';
     // ofs << "# ey" << ey.first << "    = " << ey.second << '\n';
    }
    for (const auto& ey : event.eccy())
      ofs << "# ey" << ey.first << "    = " << ey.second << '\n';

    for (const auto& farid_rm : event.rm())
      ofs << "# rm" << farid_rm.first << "    = " << farid_rm.second << '\n';
  }
...
//! <<<<<<<<<<<<<<<< Farid Added  >>>>>>>>>>>>>>>
  ...
  for (const auto& ex : event.eccx())
    hdf5_add_scalar_attr(dataset, "ex" + std::to_string(ex.first), ex.second);

  for (const auto& ey : event.eccy())
    hdf5_add_scalar_attr(dataset, "ey" + std::to_string(ey.first), ey.second);

  for (const auto& farid_rm : event.rm())
    hdf5_add_scalar_attr(dataset, "rm" + std::to_string(farid_rm.first), farid_rm.second);
}
...
Output::Output(const VarMap& var_map) {
  ...
  if (!var_map["quiet"].as<bool>() && !var_map["long-output"].as<bool>()) {
    //! ====== Farid Added
    write_header(std::cout, width);
    //! =========== Farid added
  ...
  if (!var_map["quiet"].as<bool>() && var_map["long-output"].as<bool>()) {
    //! ====== Farid Added
    write_header_long(std::cout, width);
    //! =========== Farid added
    writers_.emplace_back(
      [width](int num, double impact_param, int ncoll, const Event& event) {
        write_stream(std::cout, width, num, impact_param, ncoll, event);
